...
export const resolvers = {
    ...
    Mutation: {
        createPlayer: async (_, args, context: Context) => {
            ...
            const player = await context.db
                ...
                .findOne({ _id: getMongoID(playerResult.insertedId) });
        ...
        createCharacter: async (_, args, context: Context) => {
            const player = await context.db
                .collection(playersCollection)
                .findOne({ _id: getMongoID(args.playerID) });
            ...
            const character = await context.db
                .collection(charactersCollection)
                .findOne({ _id: characterResult.insertedId });
            player.characters.push(characterResult.insertedId);
            await context.db
                .collection(playersCollection)
                .updateOne({ _id: getMongoID(player._id) }, { $set: player });
            return {
                code: 200,
                success: true,
                message: null,
        ...
        deletePlayer: async (_, args, context: Context) => {
            if (
                await context.db
                    .collection(playersCollection)
                    .countDocuments({ _id: getMongoID(args.playerID) }, { limit: 1 })
            ) {
                const tempPlayer = await context.db
                    .collection(playersCollection)
                    .findOne({ _id: getMongoID(args.playerID) });
        ...
        deleteCharacter: async (_, args, context: Context) => {
            if (
                (await context.db
                    .collection(charactersCollection)
                    .countDocuments({ _id: getMongoID(args.characterID) }, { limit: 1 })) > 0
            ) {
                await context.db
                    .collection(playersCollection)
                    .updateOne(
                        { characters: [getMongoID(args.characterID)] },
                        { $pull: { characters: getMongoID(args.characterID) } }
                    );
                return {
                    code: 200,
                    success: true,
        ...
        deleteItem: async (_, args, context: Context) => {
            if (
                (await context.db
                    ...
                    .countDocuments({ _id: getMongoID(args.itemID) }, { limit: 1 })) > 0
            ) {
                await (context.db.collection(charactersCollection) as any).update(
                    { items: getMongoID(args.itemID) },
                    { $pull: { items: getMongoID(args.itemID) } },
                    {},
                    {}
                );
                return {
        ...
        changePlayer: async (_, args, context: Context) => {
            if (
                ...
                let tempPlayer = await context.db
                    .collection(playersCollection)
                    .findOne({ _id: getMongoID(args.playerID) });
                ...
                await context.db
                    .collection(playersCollection)
                    .updateOne({ _id: getMongoID(args.playerID) }, { $set: tempPlayer });
                return {
                    code: 200,
                    success: true,
        ...
        changeCharacter: async (_, args, context: Context) => {
            if (
                await context.db
                    .collection(charactersCollection)
                    .countDocuments({ _id: getMongoID(args.characterID) }, { limit: 1 })
            ) {
                let tempCharacter = await context.db
                    .collection(charactersCollection)
                    .findOne({ _id: getMongoID(args.characterID) });
                ...
                tempCharacter.constitution = updateValues(
                    tempCharacter.constitution,
                    args.characterData.constitution
                );
                tempCharacter.intelligence = updateValues(
                    tempCharacter.intelligence,
                    args.characterData.intelligence
                );
                ...
                await context.db
                    .collection(charactersCollection)
                    .updateOne({ _id: getMongoID(args.characterID) }, { $set: tempCharacter });
                return {
                    code: 200,
                    success: true,
        ...
        changeItem: async (_, args, context: Context) => {
            if (
                await context.db
                    .collection(itemsCollection)
                    .countDocuments({ _id: getMongoID(args.itemID) }, { limit: 1 })
            ) {
                let tempItem = await context.db
                    .collection(itemsCollection)
                    .findOne({ _id: getMongoID(args.itemID) });
                ...
                await context.db
                    .collection(itemsCollection)
                    .updateOne({ _id: getMongoID(args.itemID) }, { $set: tempItem });
                return {
                    code: 200,
                    success: true,
        ...
        characters: async (parent, _, context: Context) => {
            ...
            for (const characterID of parent.characters) {
                characters.push(
                    await context.db
                        .collection(charactersCollection)
                        .findOne({ _id: getMongoID(characterID) })
                );
            }
    ...
    Character: {
        ...
        drop: async (parent, args, context: Context) => {
            const result = await context.db
                .collection(charactersCollection)
                .updateOne({ _id: parent._id }, { $pull: { items: getMongoID(args.itemID) } });
